...
function AnimatedValue({ value, format, suffix = "" }: { value: number; format: (n: number) => string; suffix?: string }) {
...
export function TrackerCard({ stats, compact = false }: TrackerCardProps) {
  ...
  const getHoverGlow = () => {
    switch (theme) {
      case "vaporwave": return "hover:shadow-[0_0_20px_rgba(244,114,182,0.15)]"
      case "frutiger-aero": return "hover:shadow-[0_0_20px_rgba(56,189,248,0.15)]"
      case "light": return "hover:shadow-[0_0_20px_rgba(59,130,246,0.1)]"
      default: return "hover:shadow-[0_0_20px_rgba(52,211,153,0.15)]"
    }
  }
      ...
      >
        <div className="stat-item flex items-center gap-2 transition-transform duration-200 hover:scale-110">
          ...
          <span className={`text-sm font-bold ${colors.text}`}>
            <AnimatedValue value={stats.bandwidthSaved} format={formatBandwidth} />
          </span>
        </div>
        ...
        <div className="stat-item flex items-center gap-2 transition-transform duration-200 hover:scale-110">
          ...
          <span className={`text-sm font-bold ${colors.text}`}>
            <AnimatedValue value={stats.dataValueReclaimed ?? 0} format={(n) => `£${n.toFixed(2)}`} />
          </span>
        </div>
      </div>
    )
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className={`text-xs font-bold uppercase tracking-[0.2em] ${colors.textSecondary}`}>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { icon: TrendingUp, value: stats.bandwidthSaved, format: formatBandwidth, label: "Bandwidth", colorClass: colors.success },
          { icon: Clock, value: stats.timeSaved, format: formatTime, label: "Time", colorClass: colors.warning },
          { icon: PoundSterling, value: stats.dataValueReclaimed ?? 0, format: (n: number) => `£${n.toFixed(2)}`, label: "Value", colorClass: theme === "vaporwave" ? "text-pink-400" : colors.success },
        ].map((stat, i) => (
          <div
            key={i}
          ...
          >
            <div className={`p-2 rounded-full mb-3 shadow-lg ${StatIconBgColor(theme, stat.colorClass)}`}>
               <stat.icon className={stat.colorClass} size={18} />
            </div>
            <span className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${colors.textSecondary}`}>{stat.label}</span>
            <span className={`text-lg font-black tracking-tight ${colors.text} drop-shadow-sm`}>
