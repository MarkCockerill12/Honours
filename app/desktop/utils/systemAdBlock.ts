...
import { ADGUARD_DNS } from "@/lib/constants";
import { exec } from "child_process";
import { promisify } from "util";

const execAsync = promisify(exec);

// AdGuard DNS is now imported from packages/ui/constants
...
async function setAdapterDNS(
  ...
  try {
    const serverAddresses = secondaryDNS ? `'${primaryDNS}','${secondaryDNS}'` : `'${primaryDNS}'`;
    await execAsync(
      `powershell -Command "Set-DnsClientServerAddress -InterfaceAlias '${interfaceName}' -ServerAddresses ${serverAddresses}"`,
      { windowsHide: true },
    );
  ...
  try {
    ...
    for (const adapter of adapters) {
      try {
        ...
        const currentDNS = new Set(stdout
          .trim()
          .split("\n")
          .map((d) => d.trim()));
        ...
        if (
          currentDNS.has(ADGUARD_DNS.primary) ||
          currentDNS.has(ADGUARD_DNS.secondary)
        ) {
