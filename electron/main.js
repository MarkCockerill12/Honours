...
async function refreshBlocklist() {
  try {
    ...
    const lines = text
      ...
      .filter(
        (line) =>
          line.startsWith("0.0.0.0") && !line.includes("0.0.0.0 0.0.0.0"),
      )
      .map((line) => line.trim());
    ...
    console.error(
      "Failed to fetch remote blocklist, using cache or fallback:",
      err.message,
    );
    if (fs.existsSync(HOSTS_CACHE_PATH)) {
      DYNAMIC_HOSTS_BLOCKLIST = fs
        .readFileSync(HOSTS_CACHE_PATH, "utf8")
        .split("\n");
    }
  }
}
    ...
    if (fs.existsSync(DNS_BACKUP_PATH)) {
      ...
      if (Array.isArray(loaded) && loaded.length > 0) {
        ...
        originalAdapters.forEach((a) => {
          console.log(
            `Adapter: ${a.name} | V4: ${a.originalIPv4.join(",")} | V6: ${a.originalIPv6.join(",")}`,
          );
        });
      }
    }
...
async function getNetworkAdapters() {
  try {
    ...
    const getAliasesCmd =
      'powershell -Command "Get-DnsClientServerAddress | Select-Object -ExpandProperty InterfaceAlias -Unique"';
    let namesOut;
    try {
    ...
    for (const name of activeNames) {
      ...
      const { stdout: v4Out } = await execAsync(v4Cmd, {
        windowsHide: true,
      }).catch(() => ({ stdout: "" }));
      const { stdout: v6Out } = await execAsync(v6Cmd, {
        windowsHide: true,
      }).catch(() => ({ stdout: "" }));
...
async function restoreAllAdapterDNS(savedAdapters) {
  try {
    ...
    if (savedAdapters && savedAdapters.length > 0) {
      ...
      const commands = savedAdapters
        .map((adapter) => {
          let script = "";
          const name = adapter.interfaceName;

          const needsResetV4 =
            !adapter.originalIPv4 ||
            adapter.originalIPv4.includes("DHCP") ||
            cleanDnsList(adapter.originalIPv4).length === 0;
          const needsResetV6 =
            !adapter.originalIPv6 ||
            adapter.originalIPv6.includes("DHCP") ||
            cleanDnsList(adapter.originalIPv6).length === 0;
      ...
      if (commands) {
        await execAsync(`powershell -Command "${commands}"`, {
          windowsHide: true,
        });
...
async function updateHostsFile(enable) {
  try {
    ...
    if (enable) {
      ...
      const blockList =
        DYNAMIC_HOSTS_BLOCKLIST.length > 0
...
ipcMain.handle("adblock:test-dns", async () => {
  try {
    ...
    const { stdout, stderr } = await execAsync(`nslookup ${testDomain}`, {
      windowsHide: true,
      timeout: 5000,
    });
    ...
    const isNX =
      output.includes("non-existent domain") || output.includes("nxdomain");
    // Check if the server performing the lookup is AdGuard
    const isAdGuardServer =
      output.includes("94.140.") ||
      output.includes("2a10:50c0:") ||
      output.includes(":ad1:") ||
      output.includes(":ad2:");
    ...
    const isBlocked =
      isBlockedIP ||
      isNX ||
      (isAdGuardServer && output.includes("request timed out"));

    console.log(
      `[TEST] Result: ${isBlocked ? "BLOCKED" : "RESOLVING"} (Server: ${isAdGuardServer ? "AdGuard" : "Other"})`,
    );
    ...
    const isAdGuardRelated =
      msg.includes("94.140.") ||
      msg.includes("2a10:50c0:") ||
      msg.includes(":ad1:") ||
      msg.includes(":ad2:");
    const isNX =
      msg.includes("non-existent domain") || msg.includes("nxdomain");
    ...
    if (
      ...
      (isAdGuardRelated && msg.includes("refused"))
    ) {
...
ipcMain.handle("adblock:enable", async () => {
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      return {
        success: false,
        message: "Administrator privileges required. Restart as Admin.",
      };
    }
    ...
    await execAsync(`powershell -Command "${enableCommands}"`, {
      windowsHide: true,
    });

    // 3. Flush Cache
    await execAsync(
      'powershell -Command "ipconfig /flushdns; nbtstat -R; nbtstat -RR"',
      { windowsHide: true },
    ).catch(() => {});
...
ipcMain.handle("adblock:disable", async () => {
  ...
  try {
    ...
    // 2. Restore DNS
    const adaptersToRestore =
      originalAdapters.length > 0
        ? originalAdapters
        : await getNetworkAdapters();

    console.log(
      `[+] Step 3: Restoring DNS for ${adaptersToRestore.length} adapters...`,
    );
  ...
  try {
    ...
    for (const adapter of adapters) {
      ...
      const { stdout: allOut } = await execAsync(
        `powershell "Get-DnsClientServerAddress -InterfaceAlias '${adapter.interfaceName}' | Select-Object -ExpandProperty ServerAddresses"`,
        { windowsHide: true },
      );
...
function createWindow() {
  ...
  const startUrl =
    process.env.ELECTRON_START_URL || `http://localhost:3000/desktop`;
  ...
  mainWindow.webContents.on("did-fail-load", (e, code, desc) =>
    console.error("--- WEBVIEW FAIL LOAD ---", code, desc),
  );
