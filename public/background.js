  ...
  chrome.webRequest?.onBeforeRequest?.addListener(
    (details) => {
      ...
      if (check.isAd && check.category) {
        console.log(
          `[Background] Blocked ${check.category} request:`,
    ...
    async (request, sender, sendResponse) => {
      ...
      if (request.action === "TOGGLE_ADBLOCK") {
        ...
        try {
          ...
          console.log(
            "[Background] Ad block toggled successfully:",
            adBlockEnabled,
          );
      ...
      if (request.action === "TRANSLATE_TEXT") {
        ...
        console.log(
          `[Background] Translating text to ${targetLang}: "${text.substring(0, 50)}..."`,
        );
          ...
          .then((data) => {
            ...
            if (Array.isArray(data) && Array.isArray(data[0])) {
              const translatedText = data[0]
                ...
                .join("");
              console.log(
                `[Background] Translation successful: "${translatedText.substring(0, 50)}..."`,
              );
              sendResponse({ success: true, translatedText });
            } else {
              console.error(
                "[Background] Invalid translation response format:",
                data,
              );
              sendResponse({
                success: false,
      ...
      if (request.action === "CHECK_URL_REAL") {
        ...
        try {
          ...
          if (MALICIOUS_DOMAINS.has(hostname)) {
            console.log(
              `[Background] THREAT DETECTED: ${hostname} is in blocklist`,
            );
            sendResponse({
              url,
              isSafe: false,
            ...
            console.log(
              `[Background] ${hostname} not in blocklist, using heuristic scan`,
            );
