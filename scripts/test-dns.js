...
async function run() {
  ...
  try {
    ...
    const { stdout: namesOut } = await execAsync(
      "powershell -Command \"Get-NetAdapter | Where-Object { $PSItem.Status -eq 'Up' } | Select-Object -ExpandProperty Name\"",
    );
    ...
    const allServers = [
      ...ADGUARD_DNS.ipv4.map((s) => `'${s}'`),
      ...ADGUARD_DNS.ipv6.map((s) => `'${s}'`),
    ].join(",");
    const testCmd = adapters
      .map(
        (name) =>
          `Set-DnsClientServerAddress -InterfaceAlias '${name}' -ServerAddresses (${allServers})`,
      )
      .join("; ");
    ...
    for (const name of adapters) {
      const { stdout: verify } = await execAsync(
        `powershell -Command "Get-DnsClientServerAddress -InterfaceAlias '${name}' | Select-Object -ExpandProperty ServerAddresses"`,
      );
