...
const updateBlocklist = async (): Promise<{ success: boolean; error?: string }> => {
    try {
...
const setupDeclarativeNetRequest = async (): Promise<{ success: boolean; error?: string }> => {
...
if (chrome.webRequest?.onBeforeRequest) {
    chrome.webRequest.onBeforeRequest.addListener(
        (details) => {
            ...
            if (check.isAd && check.category) {
                ...
                console.log(`[Background] Tracked ${check.category} request:`, details.url.substring(0, 80));
    ...
    console.log("[Background] Web request observer registered for stats tracking (MV3 compatible)");
} else {
    console.warn("[Background] webRequest API not available — stats tracking will rely on declarativeNetRequest only");
}
...
async function handleRequest(request: any, sender: chrome.runtime.MessageSender) {
    console.log("[Background] Handling action:", request.action, "from:", sender.url);

    switch (request.action) {
        case 'GET_ADBLOCK_STATUS': {
            const status = await chrome.storage.local.get(['adBlockSetupStatus', 'adBlockSetupError', 'blocklistError']);
            return { 
                success: true, 
        ...
        case 'CHECK_URL_REAL': {
            try {
                ...
                if (MALICIOUS_DOMAINS.has(urlObj.hostname)) {
                    return { isSafe: false, threatType: 'malware', details: 'Blacklisted Domain' };
                }
        ...
        case 'QUERY_TABS': {
            return new Promise((resolve) => {
                chrome.tabs.query(request.query || {}, (tabs) => resolve({ success: true, tabs }));
            });
        }

        case 'SEND_MESSAGE': {
            return new Promise((resolve) => {
                chrome.tabs.sendMessage(request.tabId, request.message, (response: any) => resolve({ success: true, response }));
            });
        }
...
console.log("[Background] Background script initialized and ready (Dev Support Active)");
