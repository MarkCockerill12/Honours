...
export function FilterDomainExclusions() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await chromeBridge.sendMessage(undefined as any, { action: "GET_FILTER_EXCEPTIONS" });
        if (res?.success) setExceptions(res.exceptions || []);
      } catch (err) { console.warn("[FilterDomainExclusions] Load error:", err); }
    };
    load();
  }, []);
  ...
  useEffect(() => {
    if (exceptions.length > 0 && listRef.current) {
      const items = listRef.current.querySelectorAll(".fde-item");
      anime({ targets: items, translateX: [-10, 0], opacity: [0, 1], delay: anime.stagger(30), duration: 400, easing: "easeOutQuad" });
    }
  }, [exceptions.length]);

  const addException = async (domainArg?: string) => {
    let domain = (domainArg || manualDomain).trim().toLowerCase();
    if (!domain) return;
    if (!chromeBridge.isAvailable()) return;
    try {
      const res = await chromeBridge.sendMessage(undefined as any, { action: "ADD_FILTER_EXCEPTION", domain });
      if (res?.success) { setExceptions(res.exceptions); setManualDomain(""); }
    } catch (err) { console.warn(err); }
  };

  const removeException = async (domain: string) => {
    const el = document.querySelector(`[data-fde-domain="${domain}"]`);
    const doRemove = async () => {
      if (!chromeBridge.isAvailable()) return;
      try {
        const res = await chromeBridge.sendMessage(undefined as any, { action: "REMOVE_FILTER_EXCEPTION", domain });
        if (res?.success) setExceptions(res.exceptions);
      } catch (err) { console.warn(err); }
    };
    if (el) {
      anime({ targets: el, translateX: [0, 30], opacity: [1, 0], scale: [1, 0.8], duration: 300, easing: "easeInQuad", complete: () => doRemove() });
    } else {
      await doRemove();
    }
  };
  ...
  const trustCurrentSite = async () => {
    ...
    try {
      ...
      if (tabs[0]?.url) {
        ...
        if (domain && domain !== "localhost") await addException(domain);
      }
    } catch (err) { console.warn(err); }
  };
  ...
  return (
    <div className="w-full flex flex-col gap-2.5">
      <div className="flex items-center justify-between px-1 mb-1">
        <span className={`text-[10px] font-bold uppercase tracking-widest ${colors.textSecondary}`}>Filter Domain Exclusions</span>
        <button
      ...
      <div className={`${colors.bgSecondary} rounded-xl p-0.5 flex items-center gap-2 group transition-all focus-within:ring-1 focus-within:ring-primary/50`}>
        <input
          type="text"
          placeholder="Enter domain to exclude from filters..."
          value={manualDomain}
          onChange={(e) => setManualDomain(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && addException()}
          className={`bg-transparent border-none text-[11px] font-body w-full focus:ring-0 px-3 py-1.5 ${colors.text}`}
        />
        <button
      ...
      <div className="flex-1 flex flex-col gap-1 overflow-y-auto max-h-32 pr-1 custom-scrollbar" ref={listRef}>
        {exceptions.map((domain) => (
          <div key={domain} data-fde-domain={domain} className={`fde-item ${colors.bgSecondary} hover:${colors.bg} transition-colors p-2.5 rounded-lg flex items-center justify-between group border ${colors.border}`}>
