...
export default function ExtensionApp({
  ...
  return (
    <ScalableContainer
      className={`w-[400px] min-h-[600px] mx-auto overflow-hidden relative ${theme === "dark" ? "scrollbar-dark text-zinc-100" : theme === "vaporwave" ? "scrollbar-vaporwave text-zinc-100" : theme === "frutiger-aero" ? "scrollbar-frutiger text-zinc-900" : "scrollbar-light text-zinc-900"}`}
    >
      <div className={`flex flex-col h-full bg-cover bg-center ${colors.bg}`}>
        ...
        {protection.isActive && (
          <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
            ...
            <div
              className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full mix-blend-screen filter blur-[80px] bg-blue-500/20 animate-pulse-glow"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6 z-10 custom-scrollbar">
          {/* Header */}
          <div
            className={`glass-card rounded-2xl p-3 flex items-center justify-between shadow-lg
          ...
          >
            <div className="flex items-center gap-2">
              <div
                className={`p-1.5 rounded-lg ${protection.isActive ? "bg-emerald-500/20 border border-emerald-500/30" : "bg-red-500/10 border border-red-500/20"}`}
              >
                <Shield
                  className={`w-4 h-4 ${protection.isActive ? "text-emerald-400" : "text-red-400"}`}
                />
              </div>
              <h1
                className={`text-md font-black tracking-tight ${colors.text}`}
              >
                AD-BLOCK SHIELD
              </h1>
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger
                  className={`w-28 h-8 text-xs font-bold tracking-wider rounded-xl border-none ${protection.isActive ? "bg-emerald-500/10 text-emerald-500/80 hover:bg-emerald-500/20 transition-colors" : "bg-zinc-800/50 text-zinc-400"}`}
                >
                  <SelectValue />
                </SelectTrigger>
          ...
          <div className="flex flex-col items-center mt-2 space-y-8">
            ...
            <div
              className={`flex justify-center py-2 relative transition-all duration-700 ${protection.isActive ? "animate-shield-pulse" : "scale-95 grayscale-[0.2]"}`}
            >
              {protection.isActive && (
            ...
            <div className="text-center w-full max-w-[280px]">
              <h2
                className={`text-2xl font-black tracking-tighter mb-1 transition-all duration-300 ${protection.isActive ? "drop-shadow-[0_0_10px_rgba(16,185,129,0.5)]" : ""}`}
              >
                {protection.isActive ? "PROTECTED" : "EXPOSED"}
              </h2>
              <p
                className={`text-xs font-medium opacity-70 tracking-wide ${protection.isActive ? "text-emerald-400" : "text-red-400"}`}
              >
                {protection.isActive
                  ? "Network encrypted & filtered"
                  : "Ad-blocking is suspended"}
              </p>
            </div>
            ...
            <div className="w-full animate-fade-slide-up">
              <div
                className={`p-1 rounded-2xl hover-lift ${theme === "dark" ? "glass-card" : theme === "vaporwave" ? "glass-card-vaporwave" : theme === "frutiger-aero" ? "glass-card-frutiger" : "glass-card-light"}`}
              >
                <ProtectionToggles
                  protection={protection}
          ...
          {blockStats && (
            <div
              className={`rounded-xl overflow-hidden animate-fade-slide-up hover-lift shadow-lg
                ${theme === "dark" ? "glass-card" : theme === "vaporwave" ? "glass-card-vaporwave" : theme === "frutiger-aero" ? "glass-card-frutiger" : "glass-card-light"}`}
            >
              <div className="flex flex-row items-center justify-between px-4 py-3 border-b border-zinc-800/30">
                ...
                <button
                  ...
                  className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 hover:text-emerald-400 transition-colors px-2 py-1 rounded bg-black/20 hover:bg-black/40"
                >
                  Reset
                </button>
              </div>
              <div className="grid grid-cols-2 gap-px bg-zinc-800/30">
                <div className="p-3 bg-black/20 flex flex-col items-center justify-center text-center">
                  ...
                  <span className="font-black text-xl text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.3)]">
                ...
                <div className="p-3 bg-black/20 flex flex-col items-center justify-center text-center">
                  ...
                  <span className="font-black text-[15px] pt-1 text-zinc-200">
                    {(blockStats.bandwidthSaved / (1024 * 1024)).toFixed(1)}{" "}
                    <span className="text-xs text-zinc-500 ml-0.5">MB</span>
                  </span>
                </div>
                <div className="p-3 bg-black/20 flex flex-col items-center justify-center text-center">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-zinc-500 mb-1">
                  ...
                  <span className="font-black text-[15px] pt-1 text-zinc-200">
                    {blockStats.timeSaved.toFixed(0)}{" "}
                    <span className="text-xs text-zinc-500 ml-0.5">Sec</span>
                  </span>
                </div>
                <div className="p-3 bg-black/20 flex flex-col items-center justify-center text-center">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-zinc-500 mb-1">
                  ...
                  <span className="font-black text-[15px] pt-1 text-zinc-200">
                    ...
                    {blockStats.moneySaved.toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
