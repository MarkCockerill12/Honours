...
const applyFiltering = (root: Node = document.body) => {
  ...
  _isApplying = true; observer.disconnect();
  try {
    ...
    for (const textNode of textNodes) {
      ...
      if (matches && matches.length > 0) {
        ...
        if (filter.blockScope === "paragraph") {
          ...
          const unblurHandler = (e: MouseEvent) => { e.preventDefault(); e.stopPropagation(); parent.style.filter = "none"; parent.style.backgroundColor = "transparent"; parent.style.color = "inherit"; parent.style.cursor = "default"; parent.removeEventListener("click", unblurHandler, true); };
          ...
          parts.forEach(part => {
            if (part.toLowerCase() === matchedTerm) {
              ...
              if (style === "blur") span.style.filter = "blur(6px)"; else if (style === "redact") { span.style.backgroundColor = "#000"; span.style.color = "#000"; }
              const unblurWord = (e: MouseEvent) => { e.preventDefault(); e.stopPropagation(); span.style.filter = "none"; span.style.backgroundColor = "transparent"; span.style.color = "inherit"; span.style.cursor = "default"; span.removeEventListener("click", unblurWord, true); };
...
const init = async () => {
  ...
  chrome.runtime.onMessage.addListener((request, _sender, resp) => {
    ...
    if (request.action === "SCAN_PAGE_LINKS") {
      ...
      const safe = allResults.filter(r => r.isSafe).slice(0, 50); // RETURN REAL SAFE DATA
...
const performTranslation = async (targetLang: string) => {
  const now = Date.now();
  if (_isTranslationActive && (now - _translationStartTime < 30000)) return { success: false, error: "Active" };
  _isTranslationActive = true; _translationStartTime = now;
  const walk = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null);
  ...
  const uniqueTextList = Array.from(uniqueTexts); const batchSize = 50; const translationMap = new Map<string, string>();
  for (let i = 0; i < uniqueTextList.length; i += batchSize) {
    ...
    try {
      ...
      if (response?.success) batch.forEach((orig, idx) => { if (response.translatedTexts[idx]) translationMap.set(orig, response.translatedTexts[idx]); });
    } catch { }
  }
  let count = 0;
  textNodes.forEach(item => {
    ...
    if (trans && trans !== item.text && parent) {
      ...
      item.node.textContent = trans; parent.setAttribute("data-ps-translated", targetLang); count++;
    }
  });
  _isTranslationActive = false; return { success: true, count };
};
init();
