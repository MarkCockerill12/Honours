...
const CONTINENT_PATHS = {
  northAmerica: "M10,18 L12,12 L18,10 L25,12 L30,15 L32,20 L28,28 L22,32 L18,30 L14,25 L10,22 Z",
  southAmerica: "M22,38 L25,35 L30,36 L32,42 L30,50 L28,56 L24,58 L22,54 L20,48 L21,42 Z",
  europe: "M46,14 L48,11 L52,10 L56,12 L54,16 L52,18 L48,20 L46,18 Z",
  africa: "M44,24 L48,22 L54,24 L56,28 L55,36 L52,42 L48,44 L44,40 L42,34 L43,28 Z",
...
export function ServerMap({
  ...
  useEffect(() => {
    ...
    const selected = serversRef.current.querySelector(`[data-server-id="${selectedServer.id}"]`)
    if (selected) {
      anime({
        targets: selected,
        scale: [1, 1.3, 1],
  ...
  return (
    ...
    >
      ...
      >
        ...
        {GRID_LINES.map((pos) => (
          <React.Fragment key={pos}>
            <line x1={pos} y1="0" x2={pos} y2="60" stroke={getGridColor()} strokeWidth="0.3" />
            <line x1="0" y1={pos * 0.6} x2="100" y2={pos * 0.6} stroke={getGridColor()} strokeWidth="0.3" />
          </React.Fragment>
        ))}
          ...
          return (
            <button
              ...
              style={{
                ...
                boxShadow: isSelected ? `0 0 12px ${getAccentColor()}` : "0 0 4px rgba(0,0,0,0.3)",
              }}
      ...
      {/* Legend */}
      <div className={`absolute bottom-2 left-2 flex items-center gap-3 text-[10px] ${colors.textSecondary}`}>
        ...
        {isConnected && (
          <div className="flex items-center gap-1">
            <div className="w-4 h-px" style={{ background: getAccentColor() }} />
            <span>Connected</span>
          </div>
        )}
      </div>
