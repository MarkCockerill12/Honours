...
import type { ProtectionState, Theme, TrackerStats, ServerLocation } from "@/packages/ui/types"
...
export function MobileApp({
  ...
  return (
    <ScalableContainer maxWidth="420px" className="h-full flex flex-col relative overflow-hidden">
      ...
      <div className={`absolute top-0 left-0 right-0 h-1.5 ${protection.isActive ? 'animate-gradient-shift' : ''}`}
           style={{
             background: protection.isActive
               ? (theme === 'vaporwave' ? 'linear-gradient(90deg, #ec4899, #8b5cf6, #06b6d4, #ec4899)'
                  : theme === 'frutiger-aero' ? 'linear-gradient(90deg, #38bdf8, #34d399, #2dd4bf, #38bdf8)'
      ...
      <div className="flex items-center justify-between mt-4 mb-4 shrink-0 px-2">
        <div>
          ...
          <p className={`text-xs uppercase tracking-widest font-bold mt-1 ${colors.textSecondary}`}>Mobile Protection</p>
        </div>
        <button
          onClick={cycleTheme}
      ...
      {/* Content */}
      <div ref={contentRef} className="flex-1 min-h-0 flex flex-col gap-4 overflow-y-auto pb-4 px-2">
        {activeTab === "shield" && (
          <div className="animate-fade-slide-up bg-black/5 rounded-2xl h-full flex flex-col justify-center px-4 space-y-8">
            ...
            <div className="flex justify-center flex-col items-center">
              <div className={`relative ${protection.isActive ? 'animate-shield-pulse' : ''} transition-all duration-700`}>
                <ActivationButton
              ...
              {/* Status text */}
              <p className={`mt-8 text-center text-sm font-medium tracking-wide ${protection.isActive ? colors.success : colors.textSecondary}`}>
                {protection.isActive
                  ? "SYSTEM SECURED"
                  : "PROTECTION OFFLINE"}
              </p>
          ...
          <div className="animate-fade-slide-up h-full flex flex-col gap-4">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className={`text-sm font-medium uppercase tracking-wider ${colors.textSecondary}`}>Status</span>
                <span className={`text-sm font-bold tracking-wider ${isVpnConnected ? colors.success : colors.warning}`}>
                  {isVpnConnected ? "CONNECTED" : "DISCONNECTED"}
                </span>
              </div>
              <div className="flex items-center gap-3">
                <div className={`text-3xl ${colors.text}`}>{selectedServer?.flag}</div>
                <div>
                  <div className={`font-semibold ${colors.text}`}>
                    {selectedServer?.country || 'Select a server'}
                  </div>
                  <div className={`text-xs ${colors.textSecondary}`}>
                    {selectedServer && isVpnConnected ? `IP: 198.51.100.${selectedServer.ping}` : "Traffic unencrypted"}
                  </div>
                </div>
              </div>
              ...
              {isVpnConnected && (
                <div className="mt-4 pt-4 border-t border-emerald-500/20">
                  ...
                  <div className="flex justify-between mt-2 text-xs text-zinc-400">
                    ...
                    <span className="text-emerald-400 font-medium tracking-wide animate-pulse">Secured</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* VPN World Map */}
            <div className={`rounded-xl overflow-hidden hover-lift flex-1 transition-all duration-300
              ${theme === 'dark' ? 'glass-card' : theme === 'vaporwave' ? 'glass-card-vaporwave' : theme === 'frutiger-aero' ? 'glass-card-frutiger' : 'glass-card-light'}`}
            >
              <ServerMap
                servers={VPN_SERVERS}
        ...
        {activeTab === "stats" && (
          <div className="animate-fade-slide-up space-y-4 pb-4">
            ...
            {/* Additional info */}
            <div className={`p-5 rounded-xl transition-all duration-300 hover-lift
              ${theme === 'dark' ? 'glass-card' : theme === 'vaporwave' ? 'glass-card-vaporwave' : theme === 'frutiger-aero' ? 'glass-card-frutiger' : 'glass-card-light'}`}
            >
              <h3 className={`text-xs font-bold uppercase tracking-wider mb-2 ${colors.textSecondary}`}>
                How it works
              </h3>
              <p className={`text-sm ${colors.textSecondary} leading-relaxed font-medium`}>
                Privacy Shield uses system-level DNS filtering to block ads and trackers
                across all apps on your device. When enabled, ad requests are redirected to
                a null address, saving bandwidth and protecting your privacy.
              </p>
            </div>
            ...
            <div className={`p-5 rounded-xl transition-all duration-300 hover-lift
              ${theme === 'dark' ? 'glass-card' : theme === 'vaporwave' ? 'glass-card-vaporwave' : theme === 'frutiger-aero' ? 'glass-card-frutiger' : 'glass-card-light'}`}
            >
              <h3 className={`text-xs font-bold uppercase tracking-wider mb-4 ${colors.textSecondary}`}>
                System Security
              </h3>
              <div className="space-y-4">
                {[
                  { label: "Ad Blocking", active: protection.adblockEnabled, desc: "DNS-level ad filtering" },
                  { label: "VPN Tunnel", active: protection.vpnEnabled, desc: "Encrypted traffic routing" },
                  { label: "Tracker Blocking", active: protection.isActive, desc: "System-wide privacy" },
                ].map((item) => (
                  <div key={item.label} className="flex items-center justify-between pb-3 border-b border-zinc-700/50 last:border-0 last:pb-0">
                    <div>
                      <p className={`text-sm font-bold ${colors.text}`}>{item.label}</p>
                      <p className={`text-xs font-medium mt-0.5 ${colors.textSecondary}`}>{item.desc}</p>
                    </div>
                    <span className={`text-xs font-bold px-3 py-1.5 rounded-full border ${
                      item.active ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30 shadow-[0_0_10px_rgba(52,211,153,0.1)]" : "bg-zinc-800 text-zinc-500 border-zinc-700"
                    }`}>
      ...
      <div className={`mt-auto shrink-0 relative p-1.5 mx-2 rounded-2xl flex items-center shadow-[0_-10px_40px_rgba(0,0,0,0.2)] mb-4
        ${theme === 'dark' ? 'glass-card' : theme === 'vaporwave' ? 'glass-card-vaporwave' : theme === 'frutiger-aero' ? 'glass-card-frutiger' : 'glass-card-light'}`}
      >
